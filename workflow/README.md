...
文章页的“AI 整理文章资料”会根据来源链接和正文/摘录生成标题、摘要、标签、阅读时间与编辑笔记；图片页的“AI 整理图片资料”会根据选中的图片生成标题、描述、标签、情绪和使用场景。结果会先显示为一张简要复核卡，详细字段默认收起，需要调整时再展开。
...
“内容管理”只显示三个互斥的内容状态：“草稿、本地发布、云端发布”。标签页中分别显示为“草稿、待同步、云端”；“待提交、待推送、等待部署”等 Git 与 Cloudflare 进度收进详情的“同步详情”，不会和内容状态混在一起。顶部标签会显示当前筛选范围内的数量；列表默认使用紧凑模式，可选择每页 15、30 或 50 条，也可切换为摘要模式。支持当前页全选，并可继续扩展为“选择符合筛选的全部内容”；从全部结果中取消某一项时，该项会作为排除项保留。公开文章可打开本机站点页面；草稿文章可通过“真实预览”打开 `/preview/articles/<id>/`，使用与线上相同的 Astro 文章模板。

批量工具条不会因为内容状态不同而消失。“发布到本地”可直接处理所选草稿；“退回草稿”收在“更多操作”中，避免与主发布路径混在一起。服务端会先按状态统计可处理、跳过和失败数量。批量发布仍会逐条执行质量检查，失败项保持原状态，发布日期保持不变。退回草稿或删除云端已发布内容只会先改变本地站点，必须继续同步 Git 与 Cloudflare 后，云端页面才会下架。

“编辑属性”可以对所选内容追加、删除或替换标签，也可以批量修改分类、来源名称和发布日期。只有在弹窗中明确勾选的字段才会修改；发布中的内容仍会通过原有质量检查，失败项会保留选中以便调整后重试。

“同步所选”会跳过草稿；如果当前位于受保护的 `main`，管理器会创建 `content-sync/<时间>` 内容分支，只提交所选文件并推送到远程仓库。推送完成后仍需创建并合并 PR，Cloudflare 部署完成且云端内容核对通过后，状态才会变为“云端已发布”。

同步前会自动执行“内容体检”。只有“可直接上线”的内容可以进入 Git 同步；来源链接无效、来源待确认、图片文件缺失等硬错误会退回草稿，首页级来源链接、临时标题、正文过短或重复来源等情况会要求人工确认。内容管理页的“内容体检”可以查看全部结果，右上角“同步记录”保存最近的批量同步分支、提交和比较链接。
...
1. 选择“发布文章”或“发布图片”。
2. 粘贴文章正文或选择图片，再点击 AI 整理资料；先看简要结果，需要时再展开“详细资料”修改。
...
1. 在“内容管理”选择草稿，点击“复核并发布”。
2. 选择保留 flomo 原始日期，或使用今天作为正式发布日期。
3. 点击“检查并发布草稿”。前端和服务端都会强制执行文章质量检查；错误会阻止发布，重复、正文为空或标签过多等警告需要人工确认。
4. `draft: false` 只表示内容已进入本地公开站点；在管理端中会显示为“本地已发布”，并归入“待同步”标签，直到云端页面通过内容匹配验证后才显示“云端已发布”。Git 提交、推送和部署细节可在“同步详情”中查看。
