...
test("publisher exposes local public URLs and keeps empty result panels hidden", async () => {
  ...
  assert.match(html, /styles\.css\?v=20260725-image-workspace-1/);
  assert.match(html, /app\.js\?v=20260725-image-workspace-1/);
  ...
  assert.match(html, /data-article-inspector-tab="details"/);
  assert.match(html, /data-article-inspector-tab="source"/);
  assert.match(html, /data-article-inspector-tab="cover"/);
  assert.match(html, /data-article-inspector-tab="preview"/);
  assert.equal((html.match(/data-article-inspector-tab=/g) || []).length, 4);
  ...
  assert.match(html, /id="article-details-tab-status"/);
  assert.match(html, /id="article-source-tab-status"/);
  assert.match(html, /id="article-cover-tab-status"/);
  assert.match(html, /id="article-save-draft"/);
  assert.match(html, /id="article-real-preview" disabled/);
  assert.match(html, /id="article-next-action"[^>]*>发布<\/button>/);
  assert.doesNotMatch(html, /id="article-next-action"[^>]*disabled/);
  assert.match(html, /name="readTime" value="1 分钟" readonly/);
  ...
  assert.match(html, /class="article-context-ai"[^>]*id="article-title-ai"/);
  assert.match(html, /class="article-context-ai"[^>]*data-ai-fill="article"/);
  assert.ok(html.indexOf('id="article-title-ai"') < html.indexOf('id="article-title"'));
  ...
  assert.match(app, /发布会保存当前资料并执行重复与质量检查/);
  ...
  assert.match(html, /id=\"image-real-preview\" disabled/);
  ...
  assert.match(app, /\["details", "source", "cover", "preview"\]/);
  ...
  assert.match(css, /\.ai-action-menu/);
...
test("draft publishing uses explicit states, protected-branch guards, and mandatory quality checks", async () => {
  ...
  assert.match(server, /function getContentLocalPublicationStates/);
  assert.match(server, /const searchableItems = await getContentLocalPublicationStates\(indexedItems\)/);
