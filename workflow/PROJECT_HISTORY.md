...
真实批量发布暴露出最后一个流程断点：导入能够保留长段落和内部复核债务，质量门禁也能发现问题，但旧批量入口只能报错，迫使使用者逐篇整理和选择推荐分组。

批量“发布到本地”因此改为专用门禁：本地规则优先整理长段落，AI 仅作兜底并接受字符级完整性校验；内部备注在一次批量清单中明确确认；推荐分组在同一清单逐篇人工选择；提交携带检查时 SHA-256，服务端再次验证，单项失败不拖垮整批。

真实同步还发现 Markdown 链接目的地址会造成线上核验假失败。核验锚点现从渲染后可见文本提取，PR、CI、部署和线上正文继续作为四层独立事实。根因、8 条真实内容验证和防复发清单见 [`workflow/docs/BATCH_PUBLICATION_RETROSPECTIVE_2026-07-30.md`](docs/BATCH_PUBLICATION_RETROSPECTIVE_2026-07-30.md)。
...
14. **批量任务必须闭环。** 系统自动处理格式和版本问题，人工语义判断集中在同一批量清单；不能把可批量解决的问题拆成逐篇返工。
15. **核验读者可见结果。** 源 Markdown、构建产物和 HTTP 200 都不是最终内容事实；线上核验必须匹配渲染后可见文本。
